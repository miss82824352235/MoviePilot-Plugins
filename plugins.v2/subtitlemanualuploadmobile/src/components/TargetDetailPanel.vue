  ...
  <VCard class="glass-card detail-card" rounded="xl" elevation="0">
    <VCardText>
      ...
      <div class="match-panel">
        <div v-if="visibleTargets.length" class="toolbar-row batch-toolbar">
          <VBtn
          ...
          <div class="selection-summary">
            ...
            <span>已选 / {{ visibleTargets.length }} 集</span>
          </div>
          <VSpacer />
          <VMenu v-model="batchMenuOpen" location="bottom end" :close-on-content-click="true">
            ...
            <VCard class="batch-menu-card" min-width="292" rounded="xl">
              <VList density="comfortable" lines="two">
                <VListSubheader>选择后执行</VListSubheader>
                <VListItem
                  prepend-icon="mdi-upload"
                  :title="selectedTargets.length ? '上传选中字幕' : '上传整季字幕'"
                ...
                <VListItem
                  ...
                  :title="onlineBatchLabel || '搜索在线字幕'"
                  subtitle="按当前选择搜索在线字幕包"
                  :disabled="!batchUploadTargets.length || onlineSearching"
                  @click="$emit('open-batch-online-search')"
                />
                <VListItem
                  v-if="aiEnabled"
                  prepend-icon="mdi-robot-outline"
                  :title="aiBatchLabel || 'AI 生成字幕'"
                  subtitle="对可处理集数提交 AI 任务"
                  :disabled="!aiCapableBatchTargets.length || !aiAvailable || aiSubmitting"
                ...
                <VListItem
                  ...
                  :disabled="!selectedTargetIds.length || clearing"
...
.batch-menu-card {
  border: 1px solid var(--smu-border-strong);
  background: var(--smu-card-bg-strong);
  box-shadow: var(--smu-shadow);
}

.batch-menu-card :deep(.v-list-item-title) {
  font-weight: 900;
}

.batch-menu-card :deep(.v-list-item-subtitle) {
  color: var(--smu-text-muted);
  font-weight: 650;
}
