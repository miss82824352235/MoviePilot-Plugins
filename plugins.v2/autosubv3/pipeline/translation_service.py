...
class TranslationService:
    ...
    def process_batch(self, all_subs: list, batch: list, process_single=None, translate_to_zh=None) -> list:
        index_map = {id(item): idx for idx, item in enumerate(all_subs)}
        indices = [index_map[id(item)] for item in batch]
    ...
    def translate_zh_subtitle(
        ...
        try:
            ...
            if self._enable_batch():
                ...
                single = process_single or self.process_single
                processed = []
                total_items = len(valid_subs)
                for index, item in enumerate(valid_subs, start=1):
                    processed.append(single(valid_subs, item))
                    percent = 60 + int(index / max(total_items, 1) * 35)
                    self._progress_callback(percent, "translating", f"逐条翻译 {index}/{total_items}")
            self._raise_if_task_cancelled()
            ...
            self._progress_callback(96, "write_output", "保存翻译字幕")
        finally:
            self._set_output_mode(previous_output_mode)

    def translate_parallel(self, valid_subs: list, translate_to_zh=None):
        """并行批量翻译字幕，严格校验批量返回并隔离原文上下文。"""
        total = len(valid_subs)
        batch_size = self._batch_size()
        workers = self._parallel_workers()
        original_contents = [item.content for item in valid_subs]

        def original_context(target_indices: List[int], is_batch: bool) -> str:
            context_window = self._context_window()
            min_idx = max(0, min(target_indices) - context_window)
            max_idx = min(len(original_contents) - 1, max(target_indices) + context_window) if is_batch else min(target_indices)
            lines = []
        ...
        for i in range(0, total, batch_size):
            ...
            batch_map = {i + j: item for j, item in enumerate(batch_items)}
        ...
        def process_batch(batch_start_idx, batch_map):
            ...
            try:
                ...
                if not isinstance(translations, list) or len(translations) != len(indices):
                    raise ValueError(f"批量翻译返回数量不匹配 {len(translations) if isinstance(translations, list) else '非列表'}/{len(indices)}")
                if any(item is None or not self.normalize_subtitle_text_line(item) for item in translations):
                    raise ValueError("批量翻译返回包含空结果")
                for gidx, trans in zip(indices, translations):
                    item = batch_map[gidx]
                    item.content = self.format_translated_content(original_contents[gidx], trans)
                with stats_lock:
                    stats["batch_ok"] += 1
                    stats["batch_line_ok"] += len(indices)
            ...
            for gidx in indices:
                self._raise_if_task_cancelled()
                item = batch_map[gidx]
                context = original_context([gidx], is_batch=False) if self._context_window() > 0 else None
                success, trans = translate(original_contents[gidx], context, max_retries=1)
                if success and self.normalize_subtitle_text_line(trans):
                    line_ok_count += 1
                    item.content = self.format_translated_content(original_contents[gidx], trans)
                else:
                    line_failed_count += 1
                    if self._output_mode() == 'chinese_only':
                        ...
                        item.content = self.format_translated_content(original_contents[gidx], "[翻译失败]")
            with stats_lock:
                stats["fallback_line_ok"] += line_ok_count
                stats["failed_line"] += line_failed_count
                stats["batch_fail"] += 1
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                self._progress_callback(mapped_pct, "translating", f"批量翻译 {done_count}/{total} ({pct}%)")
        ...
        translated = stats["batch_line_ok"] + stats["fallback_line_ok"]
        failed = max(0, total - translated)
        self._stats()['batch_success'] = stats["batch_ok"]
        self._stats()['batch_fail'] = stats["batch_fail"]
        self._stats()['line_fallback'] = stats["fallback_line_ok"]
        self._stats()['translated'] = translated
