...
class AutoSubv3(AutoSubv3CompatMixin, _PluginBase):
    ...
    def init_plugin(self, config=None):
        ...
        self._max_segment_duration = min(
            float(config.get('max_segment_duration')) if config.get('max_segment_duration') else 5.5,
            float(self._subtitle_layout_settings["subtitle_max_duration"]),
        )
        display_capacity = max(
            8,
            int(self._subtitle_layout_settings["subtitle_max_lines"])
            * int(self._subtitle_layout_settings["subtitle_max_chars_per_line"]),
        )
        configured_max_chars = int(config.get('max_segment_chars')) if config.get('max_segment_chars') else display_capacity
        self._max_segment_chars = min(configured_max_chars, display_capacity)
        ...
        if self._translate_zh:
            ...
            # 智能助手当前 settings.LLM_* 参数，避免运行时被 MP 智能助手配置覆盖。
            runtime_llm = AutoSubTaskApi(self)._resolve_llm_runtime(config)
            openai_key = runtime_llm.get('api_key') or config.get('openai_key')
            openai_url = runtime_llm.get('translator_api_url') or runtime_llm.get('base_url') or config.get('openai_url') or "https://api.openai.com"
            openai_proxy = bool(runtime_llm.get('use_proxy'))
            openai_model = runtime_llm.get('model') or config.get('openai_model') or "inclusionAI/Ling-flash-2.0"
            compatible = bool(runtime_llm.get('translator_compatible', True))
            openai_url = str(openai_url or '').strip().rstrip('/')
            if not openai_key:
                logger.error("翻译依赖于独立 LLM API Key；已临时关闭本轮翻译能力，任务队列与监控仍继续启动")
                self._translate_zh = False
                self._openai = None
            else:
        ...
        if self._enabled:
            ...
            if not self._running:
                ...
                self._enqueue_restored_pending_tasks()
    ...
    def _get_subtitle_cleanup(self) -> SubtitleCleanupService:
        if not self._subtitle_cleanup:
    ...
    def _restore_interrupted_tasks(self) -> int:
        """恢复重载前遗留的待处理任务，避免停服时直接丢失。"""
        if not self._tasks:
            return 0
        restored = 0
        ...
        if restored:
            self.save_tasks()
            logger.info("[AutoSubv3] 已恢复 %s 个待处理/中断任务", restored)
        return restored

    def _enqueue_restored_pending_tasks(self) -> int:
        """把持久化的 pending 任务重新放回队列。"""
        if not self._tasks or not self._task_queue:
            return 0
        queued = 0
        existing = set()
        try:
            with self._task_queue.mutex:
                existing = {item.task_id for item in list(self._task_queue.queue) if item}
        except Exception:
            existing = set()
        for task in self._tasks.values():
            if task.status == TaskStatus.PENDING and not task.cancel_requested and task.task_id not in existing:
                self._task_queue.put(task)
                queued += 1
        if queued:
            logger.info("[AutoSubv3] 已重新入队 %s 个 pending 任务", queued)
        return queued
        ...
        if self._tasks is not None:
            for task_id in list(self._tasks.keys()):
                ...
                if task.status == TaskStatus.IN_PROGRESS:
                    ...
                    task.progress_updated_at = task.progress_updated_at or datetime.now()
